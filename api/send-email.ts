export default async function handler(req: any, res: any) {
  ...
  const { customerEmail, customerName, businessName, city, phone, message, items } = body;
  ...
  const messageHtml = (message && String(message).trim())
    ? `<h3 style="color:#163058">Mesazhi i klientit:</h3><p style="background:#f5f5f5;padding:12px;border-left:3px solid #163058;white-space:pre-wrap;margin:0 0 16px">${String(message).trim().replace(/</g,"&lt;").replace(/>/g,"&gt;")}</p>`
    : "";
