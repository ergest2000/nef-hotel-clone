// Krijo këtë skedar: src/lib/sendQuoteEmail.ts
...
export const sendQuoteEmail = async (payload: {
  ...
  const itemsHtml = items.map((item) =>
    `<tr>
      ...
      <td style="padding:8px;border-bottom:1px solid #eee">${item.boxes || 0}</td>
      <td style="padding:8px;border-bottom:1px solid #eee">${item.pieces || 0}</td>
    </tr>`
