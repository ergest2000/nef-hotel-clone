...
export const useProducts = (collectionId?: string) =>
  useQuery({
    ...
    queryFn: async () => {
      ...
      const primaryProducts = primary.data as Product[];
      const primaryIds = new Set(primaryProducts.map((p) => p.id));

      const extraIds = (secondary.data as any[] ?? [])
        .map((r: any) => r.product_id)
        .filter((id: string) => !primaryIds.has(id));

      if (extraIds.length === 0) return primaryProducts;
      ...
      return [...primaryProducts, ...(extraProducts as Product[])];
    },
  });
