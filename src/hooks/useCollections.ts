...
export const useUpdateGlobalColor = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<GlobalColor> }) => {
      const { error } = await supabase.from("global_colors" as any).update(updates).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["global_colors"] }),
  });
...
export const useDeleteGlobalColor = () => {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("global_colors" as any).delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["global_colors"] }),
  });
...
// ─── Product Color Assignments (many-to-many) ─────────────────────────────────
...
export const useProductColorAssignments = (productId?: string) =>
  useQuery({
    queryKey: ["product_color_assignments", productId],
    enabled: !!productId,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("product_color_assignments" as any)
        .select("*, color:global_colors(*)")
        .eq("product_id", productId!)
        .order("sort_order");
      if (error) throw error;
...
export const useAssignColorToProduct = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (item: { product_id: string; color_id: string; sort_order: number }) => {
      const { data, error } = await supabase
        .from("product_color_assignments" as any)
        .insert(item)
        .select()
    ...
    onSuccess: (_, vars) => qc.invalidateQueries({ queryKey: ["product_color_assignments", vars.product_id] }),
  });
};

export const useUnassignColorFromProduct = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ product_id, color_id }: { product_id: string; color_id: string }) => {
    ...
    onSuccess: (product_id) => qc.invalidateQueries({ queryKey: ["product_color_assignments", product_id] }),
  });
};

export const useUpdateColorAssignmentImage = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, product_id, image_url }: { id: string; product_id: string; image_url: string }) => {
      const { error } = await supabase
        .from("product_color_assignments" as any)
        .update({ image_url })
        .eq("id", id);
...
// All product color assignments (for frontend pages)
export const useAllProductColorAssignments = () =>
  useQuery({
    queryKey: ["all_product_color_assignments"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("product_color_assignments" as any)
        .select("*, color:global_colors(*)")
