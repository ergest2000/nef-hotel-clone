        ...
        <section className="py-16 md:py-24">
          <div className="container">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 md:gap-8">
              {allClients.map((name) => (
                <div key={name} className="flex flex-col items-center justify-center gap-3 p-6 border border-border hover:border-primary/30 hover:shadow-md transition-all duration-300 bg-background">
                  <Building2 size={32} className="text-muted-foreground/40" />
                  <span className="text-[11px] md:text-xs tracking-brand text-center text-muted-foreground uppercase leading-tight">{name}</span>
                </div>
              ))}
            </div>
          </div>
