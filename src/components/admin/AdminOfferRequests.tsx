...
function toFields(req: any) {
  var d = req.data as any;
  ...
  if (d.items && Array.isArray(d.items) && d.items.length > 0) {
    var itemsList = d.items.map(function (item: any) {
      var txt = item.title || "Produkt";
      if (item.code) txt += " (" + item.code + ")";
      if (item.color) txt += " — " + item.color;
      if (item.boxes) txt += " × " + item.boxes + " kuti";
      return "• " + txt;
    }).join("\n");
...
export const AdminOfferRequests = function () {
  ...
  const { data: requests, isLoading } = useQuery({
    ...
    queryFn: async function () {
      ...
      return data.filter(function (r: any) {
        var d = r.data as any;
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: function () { qc.invalidateQueries({ queryKey: ["offer_requests"] }); setDeleteId(null); },
  });
    ...
    <div>
              ...
              <div className="space-y-3">
                ...
                {d.items && Array.isArray(d.items) && d.items.length > 0 && (
                  <div>
                    ...
                    {d.items.map(function (item: any, i: number) {
                      var txt = item.title || "Produkt";
                      if (item.code) txt += " (" + item.code + ")";
                      if (item.color) txt += " — " + item.color;
                      if (item.boxes) txt += " × " + item.boxes + " kuti";
                      return <p key={i} className="text-sm text-foreground">{"• " + txt}</p>;
                    })}
                  </div>
                )}
                ...
                <Button variant="outline" size="sm" className="w-full mt-2" onClick={function () { exportToPDF("Kerkese-Oferte", [toFields(viewRecord)]); }}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {list.length > 0 && (
            <Button variant="outline" size="sm" onClick={function () { exportToPDF("Kerkesat-per-Oferta", list.map(toFields)); }}>
              <Download className="h-4 w-4 mr-2" /> Shkarko PDF
            </Button>
          )}
        </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 flex-wrap">
                      <p className="text-sm font-semibold text-foreground flex items-center gap-1.5"><User size={14} className="text-primary shrink-0" /> {d.fullName || d.business || d.email || "Pa emër"}</p>
                    ...
                    {d.items && Array.isArray(d.items) && d.items.length > 0 && (
                      <div className="mt-3 bg-primary/5 border border-primary/10 rounded p-3">
                        ...
                        {d.items.map(function (item: any, i: number) {
                          ...
                          if (item.color) txt += " — " + item.color;
                          if (item.boxes) txt += " × " + item.boxes + " kuti";
                          if (item.pieces) txt += " (" + item.pieces + " copë)";
                          return <p key={i} className="text-xs text-muted-foreground pl-4">{"• " + txt}</p>;
                        })}
                      </div>
                    )}
                  </div>
                  <div className="text-right shrink-0 flex flex-col items-end gap-2">
                    ...
                    <div className="flex gap-1">
                      <button onClick={function () { setViewRecord(req); }} className="text-muted-foreground hover:text-primary transition-colors"><Eye size={14} /></button>
                      <button onClick={function () { setDeleteId(req.id); }} className="text-destructive hover:text-destructive/80 transition-colors"><Trash2 size={14} /></button>
                    </div>
                  </div>
                </div>
              </div>
