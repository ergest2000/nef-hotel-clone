...
const generatePDF = (title: string, records: any[]) => {
  ...
  records.forEach((req: any, i: number) => {
    ...
    if (d.items && Array.isArray(d.items)) {
      ...
      d.items.forEach((item: any) => {
        content += `  • ${item.title || "Produkt"} ${item.code ? `(${item.code})` : ""} ${item.color ? `- ${item.color}` : ""} ${item.boxes ? `× ${item.boxes} kuti` : ""} ${item.pieces ? `(${item.pieces} copë)` : ""}\n`;
      });
    }
    content += `Data: ${new Date(req.created_at).toLocaleString("sq-AL")}\n\n`;
  });
...
export const AdminOfferRequests = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["offer_requests"] }); setDeleteId(null); },
  });
          ...
          {viewRecord && (() => {
            ...
            return (
              <div className="space-y-3">
                ...
                {d.items && Array.isArray(d.items) && d.items.length > 0 && (
                  <div>
                    ...
                    {d.items.map((item: any, i: number) => (
                      <p key={i} className="text-sm text-foreground">• {item.title || "Produkt"} {item.code ? `(${item.code})` : ""} {item.color ? `— ${item.color}` : ""} {item.boxes ? `× ${item.boxes} kuti` : ""}</p>
                    ))}
                  </div>
                )}
                ...
                <Button variant="outline" size="sm" className="w-full mt-2" onClick={() => generatePDF("Kerkese-Oferte", [viewRecord])}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {requests && requests.length > 0 && (
            <Button variant="outline" size="sm" onClick={() => generatePDF("Kerkesat-per-Oferta", requests)}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {d.items && Array.isArray(d.items) && d.items.length > 0 && (
                      <div className="mt-3 bg-primary/5 border border-primary/10 rounded p-3">
                        ...
                        {d.items.map((item: any, i: number) => (
                          <p key={i} className="text-xs text-muted-foreground pl-4">• {item.title || "Produkt"} {item.code ? `(${item.code})` : ""} {item.color ? `— ${item.color}` : ""} {item.boxes ? `× ${item.boxes} kuti` : ""} {item.pieces ? `(${item.pieces} copë)` : ""}</p>
                        ))}
                      </div>
                    )}
                  </div>
