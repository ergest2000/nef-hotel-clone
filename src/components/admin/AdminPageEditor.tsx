...
const SortableSection = ({
  ...
  const storageKey = "admin_expanded_" + section.page + "_" + section.section_key;
  const [expanded, setExpandedRaw] = useState(function () {
    try { return sessionStorage.getItem(storageKey) === "true"; } catch { return false; }
  });
  const setExpanded = function (val: boolean) {
    try { sessionStorage.setItem(storageKey, String(val)); } catch {}
    setExpandedRaw(val);
  };
