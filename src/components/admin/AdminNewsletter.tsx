...
export const AdminNewsletter = function () {
  ...
  const { data: subscribers, isLoading } = useQuery({
    ...
    queryFn: async function () {
      ...
      return data.filter(function (r: any) { return (r.data as any).type === "newsletter"; });
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: function () { qc.invalidateQueries({ queryKey: ["newsletter_from_registrations"] }); setDeleteId(null); },
  });
        ...
        <div className="flex items-center gap-3">
          ...
          {list.length > 0 && (
            <Button variant="outline" size="sm" onClick={function () {
              exportToPDF("Newsletter", list.map(function (s: any) {
                return [
      ...
      {list.length === 0 ? (
        ...
        <div className="border border-border rounded-lg overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {list.map(function (sub: any, i: number) {
