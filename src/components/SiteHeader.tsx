...
const SearchDropdown = ({ query, isAl, onSelect }: { query: string; isAl: boolean; onSelect: () => void }) => {
  ...
  return (
    <div className="absolute top-full left-0 right-0 mt-1 bg-background border border-border rounded-lg shadow-lg z-50 max-h-[400px] overflow-y-auto">
      {results.length === 0 ? (
        ...
        <div>
          {results.map((product) => (
            <button key={product.id} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-muted/50 transition-colors text-left border-b border-border last:border-b-0" onClick={() => { navigate(`/koleksionet/${product.collectionSlug}/${product.id}`); onSelect(); }}>
    ...
    <div className="absolute top-full right-0 mt-2 w-72 bg-background border border-border rounded-lg shadow-xl z-50 overflow-hidden">
        ...
        <div className="border-b border-border">
          ...
          <div className="max-h-[160px] overflow-y-auto">
            {offers.slice(0, 5).map((offer) => (
              <button key={offer.id} onClick={() => { if (offer.product_id) navigate(`/koleksionet/all/${offer.product_id}`); onClose(); }} className="w-full flex items-start gap-3 px-4 py-2.5 hover:bg-muted/50 transition-colors text-left">
                <Tag size={14} className="text-primary mt-0.5 shrink-0" />
                <div className="min-w-0">
                  <p className="text-xs font-medium text-foreground truncate">{isAl ? offer.title_al : offer.title_en || offer.title_al}{offer.discount_percent > 0 && (<span className="ml-1.5 text-primary font-bold">-{offer.discount_percent}%</span>)}</p>
      ...
      <div className="py-1">
        {menuItems.map(({ label, icon: Icon, action }) => (
          <button key={label} onClick={() => { action(); onClose(); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-foreground hover:bg-muted/50 transition-colors text-left">
            <Icon size={16} className="text-muted-foreground" />{label}
          </button>
        ))}
      </div>
      <div className="border-t border-border py-1">
        <button onClick={async () => { await signOut(); onClose(); navigate("/"); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-destructive hover:bg-muted/50 transition-colors text-left">
      ...
      <div className="hidden lg:block">
        <div className="border-b border-border">
          <div className="container flex items-center h-12 gap-8 text-[13px] tracking-brand">
            ...
            <div className="relative flex-1 mx-auto max-w-sm" ref={searchRef}>
              <input type="text" placeholder={isAl ? "Kerko per produkte ketu" : "Search for products here"} className="w-full h-8 pl-4 pr-9 text-[13px] border border-border bg-background rounded-full focus:outline-none focus:ring-1 focus:ring-primary placeholder:text-muted-foreground/60 text-center placeholder:text-center" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onFocus={() => setSearchFocused(true)} />
            ...
            <div className="flex items-center gap-7 shrink-0 ml-auto">
              ...
              {user ? (
                <div className="relative" ref={profileRef}>
                  <button onClick={() => setProfileOpen(!profileOpen)} className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors px-3">
        ...
        <div className="border-b border-border">
          <div className="container flex items-center h-20 gap-4">
            ...
            <button className="text-foreground hover:text-primary transition-colors shrink-0" onClick={() => setDesktopMenuOpen(!desktopMenuOpen)}>{desktopMenuOpen ? <X size={24} /> : <Menu size={24} />}</button>
            <nav className="flex items-center gap-4 xl:gap-5">
              {productLinks.map((item) => (<SlugLink key={item.label} to={item.href} className="text-[10px] xl:text-xs tracking-brand text-muted-foreground hover:text-primary transition-colors uppercase whitespace-nowrap">{item.label}</SlugLink>))}
            </nav>
          </div>
        </div>
        {desktopMenuOpen && (
          <div className="border-b border-border bg-background shadow-sm">
            <div className="container py-6">
              <div className="flex flex-col gap-1">
                {mainLinks.map((item) => (<SlugLink key={item.label} to={item.href} onClick={() => setDesktopMenuOpen(false)} className="text-xs tracking-brand text-muted-foreground hover:text-primary transition-colors uppercase py-2">{item.label}</SlugLink>))}
              </div>
            </div>
          </div>
        )}
      ...
      <div className="lg:hidden">
        <div className="border-b border-border">
          <div className="container flex items-center justify-between h-14">
            <button className="text-foreground" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>{mobileMenuOpen ? <X size={22} /> : <Menu size={22} />}</button>
        ...
        <div className="border-b border-border px-4 py-2" ref={mobileSearchRef}>
          <div className="relative w-full">
            <input type="text" inputMode="search" placeholder={isAl ? "Kërko për produkte këtu..." : "Search for products here..."} className="w-full h-11 pl-4 pr-12 text-[16px] border border-border bg-background rounded-full focus:outline-none focus:ring-1 focus:ring-primary placeholder:text-muted-foreground/50" value={mobileSearchQuery} onChange={(e) => setMobileSearchQuery(e.target.value)} onFocus={() => setMobileSearchFocused(true)} />
            ...
            <div className="container py-3 flex flex-col gap-1">
              ...
              {mobileProductsOpen && (
                <div className="flex flex-col gap-1 pl-4">
                  {productLinks.map((item) => (<SlugLink key={item.label} to={item.href} onClick={() => { setMobileMenuOpen(false); setMobileProductsOpen(false); }} className="text-sm tracking-brand text-muted-foreground/80 hover:text-primary transition-colors uppercase py-2 border-b border-border/50 last:border-b-0">{item.label}</SlugLink>))}
                </div>
              )}
              {mainLinks.map((item) => (<SlugLink key={item.label} to={item.href} onClick={() => setMobileMenuOpen(false)} className="text-sm tracking-brand text-muted-foreground hover:text-primary transition-colors uppercase py-2.5 border-b border-border last:border-b-0">{item.label}</SlugLink>))}
